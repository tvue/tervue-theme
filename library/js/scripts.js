...
$(document).ready(function(){
     ...
     $('.close').click(function(){
      ...
      $('.title-box').velocity("transition.shrinkIn", 1500);
       $('.header-background-image').velocity("transition.fadeIn", 1000);    
   //$('.mainPage').velocity("transition.slideDownBigIn", 800);

                var row = $('div.row'),
                    rowOffset = row.offset().top/2,
                    doc= $(document);
                
                doc.on('scroll', function() {
                    if ( doc.scrollTop() > rowOffset && row.hasClass('hidden') )
                    { 

                    row.removeClass('hidden'); 
                    $('.column').velocity("transition.bounceIn", {stagger:250});
          }
                });
